...
bool noSystematics = false;              // force all syiistematics to be off (i.e. set all nuisance parameters as constat
...
int  printLevel=0;
bool generateBinned = true;             // for binned generation
...
*/
  ...
  if (!bModel ) {
   ...
   else if (calcType == 2) hypoCalc= new  AsymptoticCalculator(*data, *sbModel, *bModel, false);
   ...
   if (sampler && (calcType == 0 || calcType == 1)) {
      ...
      cout << "Setting test statistics" << endl;
      cout << "Bg extended: " << bModel->GetPdf()->canBeExtended() << endl;
      cout << "Combined extended: " << sbModel->GetPdf()->canBeExtended() << endl;
...
void cl_hypotest() {
    StandardHypoTestDemo("model_OPT089W3TeV.root", "model", "bg+sModel", "bgModel", "model_hist", 0, 2, 5000, false);
}
